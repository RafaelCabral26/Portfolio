...
export function ProjectsPage() {
    return (
        <div id="projects" className="h-[90%] sm:h-screen flex flex-col  py-10   ">
            <div id="divisor" className="w-[30%] h-1 border-b-2 border-white mx-10 mt-32"></div>
            <div className="m-auto w-full sm:w-[80%] md:w-[50%] lg:w-[100%] xl:w-[80%] 2xl:w-[60%] my-10  rounded-lg   ">
                <Swiper
                    pagination={{
                        dynamicBullets: true,
                    }}
            ...
            <div id="divisor" className="w-[30%] h-1 border-b-2 border-white mx-10 self-end mb-20 lg:mb-64"></div>
        </div>
    );
}
function ProjectsInfo({ tecs, gitHub, webSiteLink, description }) {
    return (
        <div className="absolute  top-0 lg:top-[25%] lg:left-[5%]   w-full lg:w-[50%] h-[90%] lg:h-[52%] flex flex-col text-center 2xl:text-start justify-between rounded-lg lg:bg-secondary lg:pb-10  ">
            <ProjectsLinks gitHub={gitHub} webSiteLink={webSiteLink}></ProjectsLinks>
            <span className=" hidden  lg:flex self-end 2xl:self-center text-start -mt-10 mb-6 pr-10  ">{description}</span>
            <div className="self-center flex">
                {tecs.split(/,\s*/).map((e) => {
                    return <ProjectLabel tec={e} key={e}></ProjectLabel>;
                })}
            </div>
        </div>
    );
...
function ProjectLabel({ tec }) {
    return (
        <div className="px-2 py-1   border rounded-md bg-secondary bg-opacity-90 mx-1 ">
            <span className="text-accent "> {tec}</span>
        </div>
    );
}
function ProjectsLinks({ gitHub, webSiteLink }) {
    return (
        <div className="flex flex-col items-start gap-2  m-2">
            <a href={gitHub} target="_blank">
                <GithubLinkSvg></GithubLinkSvg>
            </a>
            <a href="#">
                <WebsiteLinkSvg></WebsiteLinkSvg>
            </a>
        </div>
    );
}

function GithubLinkSvg() {
    return (
        <svg className="w-8 group" viewBox="0 0 73 73" version="1.1" fill="#000000" >
            ...
            <g id="SVGRepo_iconCarrier">
                ...
                <g id="team-collaboration/version-control/github" stroke="none" strokeWidth="1" fill="none" fillRule="evenodd">
                    ...
                    <g id="container" transform="translate(2.000000, 2.000000)" fillRule="nonzero">
                        {" "}
                        <rect id="mask" strokeWidth="2" fill="#2D3540" className="fill-secondary opacity-80" x="-1" y="-1" width="71" height="71" rx="14">
                            {" "}
                        </rect>{" "}
...
function WebsiteLinkSvg() {
    return (
        <svg className="w-8 group bg-secondary rounded-md opacity-80" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
            <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
            <g id="SVGRepo_tracerCarrier" strokeLinecap="round" strokeLinejoin="round"></g>
            <g id="SVGRepo_iconCarrier">
                {" "}
                <path  className="group-hover:stroke-accent"
